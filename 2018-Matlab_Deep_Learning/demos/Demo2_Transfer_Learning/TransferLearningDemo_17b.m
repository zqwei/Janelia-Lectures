...
%% Load Image Data
% We will use data from the Food101 public dataset, available at https://www.vision.ee.ethz.ch/datasets_extra/food-101/
% Download the dataset and extract the following categories from the archive:
% - caesar_salad
% - french_fries
% - hamburger
% - pizza
% - sushi
% Place them in the foodData/train directory.
...
imagepath = fullfile('foodData', 'train');
...
if doTrain
    ...
    playVideo(fullfile('results','tfLearning1.mp4'))
    figure, imshow(fullfile('results','tfLearning1.png'))
    load(fullfile('results','firstPassNetwork.mat'));
end
...
if doTest
    ...
    load(fullfile('results','test1.mat'));
end
...
imshow(im_display);
title(sprintf('%s %.2f, actual ', char(label),max(conf),char(actualLabe)),'interpreter', 'none');
...
% the training. Let's make the trained network less 'black-box' by diving
% into it more.
VisualizeActivations;

%% (optional) Image Data Augmentation
% To prevent overfitting and improve training results, oftentimes data
% augmentation is needed.

% For images, there are many different data augmentations:
% random crop
% random left-right flip
% random transforms (shear, stretch, rotation, scaling, etc)


% Let's create an image augmenter that preprocesses images before training.
% This augmenter resizes and reflects images at random.
% imageAugmenter = imageDataAugmenter('RandRotation',[-10 10],'FillValue',50);
...
augmenter = imageDataAugmenter( ...
    ...
    'RandYScale',[1 2])
datasource = augmentedImageSource(outputSize,trainDS,...
    'DataAugmentation',augmenter,...
    'ColorPreprocessing','gray2rgb');

if doTrain
    % Now retrain the model with augmented data
    tic
    net = trainNetwork(datasource, layers_train, opts);
    toc
else
    playVideo(fullfile('results','tfLearning2.mp4'))
    figure, imshow(fullfile('results','tfLearning2.png'))
    load(fullfile('results','secondPassNetwork.mat'));
end

accuracy_augment = validatePerformance(net,testDS,doTest)
...
if doTrain
    ...
    % Here I will proceed with the set of best points, returned by:
    ...
    % performance check
    accuracy_bayesopt = validatePerformance(net,testDS)
    
else
    playVideo(fullfile('results','tfLearning3_bayes.mp4'));
    open(fullfile('results','bayesOptResult.fig'))
end
